...
export function DashboardPage() {
  ...
  const pipelineStages = pipeline.emailPipeline?.length ? pipeline.emailPipeline : [
    { label: 'Email intake', value: toArray<any>(data.workflows)?.[0]?.total ?? toArray<any>(data.metrics)?.[0]?.value ?? 0, detail: 'messages received' },
    { label: 'AI classified', value: toArray<any>(data.workflows)?.[0]?.active ?? toArray<any>(data.metrics)?.[1]?.value ?? 0, detail: 'awaiting routing' },
    { label: 'RFQs created', value: toArray<any>(data.recentRfqs)?.length ?? 0, detail: 'latest records' },
    { label: 'Human review', value: toArray<any>(data.workflows)?.find((workflow) => workflow.name.toLowerCase().includes('review'))?.active ?? toArray<any>(data.metrics)?.[2]?.value ?? 0, detail: 'operator decisions' },
  ];
  return <div className="fade-up">
    <PageHeading eyebrow="Operations / overview" title="Good afternoon, Alex" description="Your operational picture for the current shift." action={<div className="flex items-center gap-3">
      {demoModeEnabled && (
        <div className="flex items-center gap-2">
          ...
          <button data-testid="button-seed-demo-dashboard" onClick={seedDemo} disabled={seeding} className="inline-flex items-center justify-center gap-2 rounded-md border border-indigo-200 bg-indigo-50 px-3 py-2 text-[10px] font-extrabold text-indigo-700 transition-transform hover:-translate-y-px disabled:cursor-not-allowed disabled:opacity-50">
            {seeding ? 'Loading...' : 'Load Demo Emails'}
          </button>
        </div>
      )}
      <Link href="/rfq-inbox" data-testid="link-view-queue" className="inline-flex items-center gap-2 rounded-md bg-[hsl(var(--primary))] px-4 py-2.5 text-[11px] font-extrabold text-[hsl(var(--primary-foreground))] shadow-sm transition-transform hover:-translate-y-0.5">Open RFQ queue <ArrowRight size={14} /></Link>
    </div>} />
    <div className="mb-8 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {toArray<any>(data.metrics).map((metric, index) => <div key={`${metric.label}-${index}`} data-testid={`card-metric-${index}`} className={`fade-up fade-up-delay-${Math.min(index + 1, 3)} group relative overflow-hidden rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 shadow-[0_1px_1px_hsl(var(--primary)/.03)]`}>
    ...
    <div className="grid gap-6 xl:grid-cols-[1.55fr_1fr]">
      <section>
        <SectionTitle title="Recent RFQs" meta={`${toArray<any>(data.recentRfqs).length} latest`} action={<Link href="/rfq-inbox" data-testid="link-all-rfqs" className="flex items-center gap-1 text-[11px] font-bold text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--primary))]">View all <ArrowRight size={13} /></Link>} />
        <div className="overflow-hidden rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))]">
          <RfqTable rfqs={toArray<any>(data.recentRfqs).slice(0, 6)} compact />
        </div>
      </section>
      <section>
        <SectionTitle title="Workflow pulse" meta="live" />
        <div className="space-y-3">
          {toArray<any>(data.workflows).map((workflow) => <div key={workflow.name} className="rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-4"><div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2.5"><ToneDot tone={workflow.tone} /><span className="text-[12px] font-bold">{workflow.name}</span></div><span className="mono text-[10px] text-[hsl(var(--muted-foreground))]">{workflow.active} active</span></div><div className="flex items-center gap-3"><div className="h-1.5 flex-1 overflow-hidden rounded-full bg-[hsl(var(--muted))]"><div className="h-full rounded-full bg-[hsl(var(--accent))]" style={{ width: `${workflow.progress}%` }} /></div><span className="mono w-8 text-right text-[10px] font-medium">{workflow.progress}%</span></div><div className="mt-2 text-[10px] text-[hsl(var(--muted-foreground))]">{workflow.total} total requests in flight</div></div>)}
        </div>
      </section>
    </div>
...
export function EmailInboxPage() {
  ...
  const records = toArray<typeof emails.data extends Array<infer U> ? U : any>(emails.data).filter((email) => `${email.sender} ${email.subject} ${email.senderEmail}`.toLowerCase().includes(emailSearch.toLowerCase()));
  const active = records.find((email) => email.id === selected) ?? records[0];
  return <div className="fade-up"><PageHeading eyebrow="Command center / intake" title="Email inbox" description="Incoming customer traffic, enriched by the classification agent." action={<div className="flex items-center gap-2 text-[10px] text-[hsl(var(--muted-foreground))]">
    {demoModeEnabled && (
      <button data-testid="button-seed-demo-inbox" onClick={seedDemo} disabled={seeding} className="mr-2 inline-flex items-center justify-center gap-2 rounded-md border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-[10px] font-extrabold text-indigo-700 transition-transform hover:-translate-y-px disabled:cursor-not-allowed disabled:opacity-50">
        {seeding ? 'Loading...' : 'Load Demo Emails'}
      </button>
    )}
    <Mail size={14} className="text-[hsl(var(--accent))]" /> {records.length} messages
...
export function AiReviewPage() {
  ...
  const records = toArray<typeof reviews.data extends Array<infer U> ? U : any>(reviews.data);
